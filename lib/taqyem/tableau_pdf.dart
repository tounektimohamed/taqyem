...
class HTMLReportGenerator {
  static Future<void> generateAndDownloadReport({
    ...
    // Nouveaux paramètres optionnels (ne cassent pas l'existant)
    String selectedClass = '',
    List<Map<String, dynamic>> criteria = const [],
  }) async {
    try {
      // Charger le logo en base64 (code existant)
      String logoBase64 = "";
      try {
  ...
  static String _buildCompleteHTMLContent({
    ...
    // Générer le tableau des critères (nouveau - optionnel)
    final criteriaHTML = criteria.isNotEmpty
        ? _buildCriteriaHTML(
            criteria: criteria, t: t, isFrenchInterface: isFrenchInterface)
        : '';
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Page des données de période (NOUVELLE) */
        .period-page {
            background: var(--white);
            border-radius: var(--border-radius);
            box-shadow: var(--box-shadow);
        ...
        @media print {
            * {
            ...
            /* Page des données de période pour impression */
            .period-page {
                box-shadow: none !important;
                border: none !important;
                padding: 0 !important;
            ...
            /* Page des critères pour impression */
            .criteria-page {
                box-shadow: none !important;
                border: none !important;
                padding: 0 !important;
            ...
            .period-page, .criteria-page, .table-page {
        ...
        @media screen and (max-width: 768px) {
            ...
            .cover-page, .period-page, .table-page, .criteria-page {
                padding: 20px;
            }
...
<body>
    <div class="report-container">
        ...
        <!-- 2. Page des données de période -->
        ${_buildPeriodDataPageHTML(trimestre, periode, evaluationType, t, isFrenchInterface, matiereName, className, profName)}
        
        <!-- 3. Page du tableau des critères (si disponibles) -->
        ${criteriaHTML}
        
        <!-- 4. Page du tableau d'évaluation -->
  ...
  static String _buildPeriodDataPageHTML(
    ...
    <div class="period-page">
        <div class="table-header">
            <h2 class="table-main-title">
                ${isFrenchInterface ? 'Informations de la période' : 'معلومات الفترة'}
            </h2>
        </div>
        ...
        <div class="subject-info">
            <h3 style="font-size: 20px; margin-bottom: 15px; text-align: center;">
                ${isFrenchInterface ? 'Résumé de l\'évaluation' : 'ملخص التقييم'}
            </h3>
            
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 15px;">
                <div>
                    <div style="font-size: 14px; opacity: 0.9;">${isFrenchInterface ? 'Matière' : 'المادة'}</div>
                    <div style="font-size: 18px; font-weight: bold;">$matiereName</div>
                </div>
                
                <div>
                    <div style="font-size: 14px; opacity: 0.9;">${isFrenchInterface ? 'Classe' : 'القسم'}</div>
                    <div style="font-size: 18px; font-weight: bold;">$className</div>
                </div>
                
                <div>
                    <div style="font-size: 14px; opacity: 0.9;">${isFrenchInterface ? 'Professeur' : 'الأستاذ'}</div>
                    <div style="font-size: 18px; font-weight: bold;">$profName</div>
                </div>
            </div>
        </div>
  ...
  static String _buildCoverPageHTML(
    ...
    <div class="cover-page">
        ...
        <div class="info-grid">
            <div class="info-card">
                <div class="info-content">
                    <div class="info-label">${t['trimestre']}</div>
                    <div class="info-value">${_getTrimestreDisplay(trimestre, isFrenchInterface)}</div>
                </div>
            </div>
            
            <div class="info-card">
                <div class="info-content">
                    <div class="info-label">${t['periode']}</div>
            ...
            <div class="info-card">
                <div class="info-content">
                    <div class="info-label">${t['evaluation_type']}</div>
                    <div class="info-value">${_getEvaluationTypeDisplay(evaluationType, isFrenchInterface)}</div>
                </div>
            </div>
        </div>
        ''' : ''}

...
//   static final StudentsCacheManager _instance =
...
//         .where((key) =>
//             key.contains('students_$userId') && key.contains('_$classId'))
//         .toList();
...
//       String userId, String classDocId,
...
//   static Future<String?> getSousBaremeEvaluationSystemWithCache(String userId,
//       String classId, String matiereId, String baremeId, String sousBaremeId,
//       {bool forceRefresh = false}) async {
//     final cacheKey =
//         'sous_bareme_system_${userId}_${classId}_${matiereId}_${baremeId}_$sousBaremeId';
...
//           .collection('users')
//           .doc(userId)
//           .collection('sous_bareme_evaluation_systems')
//           .doc('$classId-$matiereId-$baremeId-$sousBaremeId')
//           .get();

//       String? system;
//       if (doc.exists && doc.data()?['system'] != null) {
//         system = doc.data()!['system'] as String;
//       }

//       _cache.addToCache(cacheKey, system);
//       return system;
//     } catch (e) {
//       print('Erreur chargement système sous-barème: $e');
//       return null;
//     }
//   }

// // AJOUTER CETTE MÉTHODE À LA CLASSE StudentsFirestoreHelper DANS LE CODE 3
//   static Future<List<QueryDocumentSnapshot>> getSousBaremesList(String userId,
//       String selectedClass, String selectedMatiere, String baremeId,
//       {bool forceRefresh = false}) async {
//     final cacheKey =
//         'sousbaremes_${userId}_${selectedClass}_${selectedMatiere}_$baremeId';
...
//       String userId, String selectedClass, String selectedMatiere,
//       {bool forceRefresh = false}) async {
...
//       String userId, String classDocId, String studentId, String baremeId,
//       {bool forceRefresh = false}) async {
...
//   static Future<DocumentSnapshot> getSousBaremeMarksWithCache(String userId,
//       String classDocId, String studentId, String baremeId, String sousBaremeId,
//       {bool forceRefresh = false}) async {
//     final cacheKey =
//         'sousmarks_${userId}_${studentId}_${baremeId}_$sousBaremeId';
...
//   static Future<DocumentSnapshot> getClassDocument(String classId,
//       {bool forceRefresh = false}) async {
...
//       String classId, String matiereId,
//       {bool forceRefresh = false}) async {
...
//       String userId, String classId, String matiereId,
//       {bool forceRefresh = false}) async {
...
//       String userId, String classId, String matiereId,
//       {bool forceRefresh = false}) async {
//     final cacheKey = 'global_custom_notes_${userId}_${classId}_$matiereId';
...
//   static Future<List<String>> getBaremeCustomNotesWithCache(
//       String userId, String classId, String matiereId, String baremeId,
//       {bool forceRefresh = false}) async {
//     final cacheKey =
//         'bareme_custom_notes_${userId}_${classId}_${matiereId}_$baremeId';
...
//   static Future<List<String>> getSousBaremeCustomNotesWithCache(String userId,
//       String classId, String matiereId, String baremeId, String sousBaremeId,
//       {bool forceRefresh = false}) async {
//     final cacheKey =
//         'sous_bareme_custom_notes_${userId}_${classId}_${matiereId}_${baremeId}_$sousBaremeId';
...
//   String _getDisplayEvaluationForTable(String storedValue, String system,
...
//       // Vérifier si storedValue est déjà une note personnalisée
//       for (String note in customNotes) {
//         if (storedValue == note) {
//           return note;
//         }
//       }

//       // Convertir le caractère en note personnalisée
//       if (storedValue == '( - - - )') {
...
//             print(
//                 'Erreur dans _buildMarkCell pour $baremeKey: ${snapshot.error}');
//           }

//           final value = snapshot.data ?? _dropdownValues[0];
//           print('Cellule $baremeKey: $value');
...
//     final cacheKey =
//         'baremes_values_${widget.currentUser.uid}_${widget.selectedClass}_${widget.selectedMatiere}';
...
//     );
//   }
// // Mettre à jour la fonction _getDisplayEvaluation dans le code 3
// // REMPLACER LA FONCTION EXISTANTE PAR CE CODE

// // CORRECTION COMPLÈTE DE _getDisplayEvaluation
...
//     // 2. SYSTÈME PERSONNALISÉ - Afficher la NOTE PERSONNALISÉE
//     if (system == 'custom' && customNotes != null && customNotes.isNotEmpty) {
//       // Si la valeur stockée est déjà une note personnalisée
//       for (String note in customNotes) {
//         if (storedValue == note) {
//           return note;
//         }
...
//       // Sinon, convertir le caractère en note personnalisée
...
// // CORRECTION DE _getSelectedValue

// Future<String> _getSelectedValue(String studentId, String baremeKey) async {
//   try {
//     // Récupérer le système GLOBAL
//     final String evaluationSystem = await StudentsFirestoreHelper
//         .getEvaluationSystemWithCache(
//       widget.currentUser.uid,
//       widget.selectedClass,
...
//     print('''
// === DEBUG _getSelectedValue ===
// Student: $studentId
// Bareme Key: $baremeKey
// Global System: $evaluationSystem
// ''');
    
//     // DÉTERMINER SI C'EST UN BARÈME OU UN SOUS-BARÈME
//     // Basé sur votre log, vérifier si c'est un sous-barème connu
//     bool isKnownSousBareme = false;
//     String? parentBaremeId;
//     String? sousBaremeId;
    
//     // Liste des sous-barèmes connus d'après vos logs
//     final knownSousBaremes = {
//       '5QYUZeTCGOCjZhLVGaWl': '4xa2SUo3qV5uGA0TbSXd',
//       'hKBd7dUHoporCwSo5Kw9': '4xa2SUo3qV5uGA0TbSXd',
//     };
    
//     if (knownSousBaremes.containsKey(baremeKey)) {
//       isKnownSousBareme = true;
//       parentBaremeId = knownSousBaremes[baremeKey];
//       sousBaremeId = baremeKey;
//       print('⚠️ DÉTECTÉ: Sous-barème connu - Parent: $parentBaremeId, Sous: $sousBaremeId');
//     } else if (baremeKey.contains('-')) {
//       var parts = baremeKey.split('-');
//       if (parts.length >= 2) {
//         parentBaremeId = parts[0];
//         sousBaremeId = parts[1];
//         isKnownSousBareme = true;
//         print('⚠️ DÉTECTÉ: Sous-barème avec ID composé - Parent: $parentBaremeId, Sous: $sousBaremeId');
//       }
//     }
    
//     // CHARGER LES NOTES PERSONNALISÉES
//     List<String> customNotes = [];
    
//     if (evaluationSystem == 'custom') {
//       if (isKnownSousBareme && parentBaremeId != null && sousBaremeId != null) {
//         // 1. ESSAYER LES NOTES SPÉCIFIQUES AU SOUS-BARÈME
//         customNotes = await StudentsFirestoreHelper
//             .getSousBaremeCustomNotesWithCache(
//           widget.currentUser.uid,
//           widget.selectedClass,
//           widget.selectedMatiere,
//           parentBaremeId,
...
//         print('📋 Notes spécifiques sous-barème ($sousBaremeId): $customNotes');
        
//         // 2. SI VIDE, ESSAYER LES NOTES DU BARÈME PARENT
//         if (customNotes.isEmpty) {
//           customNotes = await StudentsFirestoreHelper
//               .getBaremeCustomNotesWithCache(
//             widget.currentUser.uid,
//             widget.selectedClass,
//             widget.selectedMatiere,
//             parentBaremeId,
//           );
//           print('📋 Fallback notes barème parent ($parentBaremeId): $customNotes');
//         }
        
//         // 3. SI TOUJOURS VIDE, UTILISER LES NOTES GLOBALES
//         if (customNotes.isEmpty) {
//           customNotes = await StudentsFirestoreHelper
//               .getCustomNotesWithCache(
//             widget.currentUser.uid,
...
//             widget.selectedMatiere,
//           );
//           print('📋 Notes globales: $customNotes');
//         }
//       } else {
...
//           baremeKey,
//         );
//         print('📋 Notes barème principal ($baremeKey): $customNotes');
        
//         // SI VIDE, UTILISER LES NOTES GLOBALES
//         if (customNotes.isEmpty) {
//           customNotes = await StudentsFirestoreHelper
//               .getCustomNotesWithCache(
//             widget.currentUser.uid,
...
//       if (customNotes.isEmpty) {
//         customNotes = ['0', '1', '2', '3'];
//         print('📋 Notes par défaut: $customNotes');
//       }
//     }
...
//     if (isKnownSousBareme && parentBaremeId != null && sousBaremeId != null) {
//       // RECHERCHE POUR LES SOUS-BARÈMES
      
//       // 1. Chercher directement par ID du sous-barème
//       var sousBaremeDirectDoc = await StudentsFirestoreHelper
//           .getStudentMarksWithCache(
//         widget.currentUser.uid,
//         widget.classDocId,
//         studentId,
//         sousBaremeId,
...
//       if (sousBaremeDirectDoc.exists) {
//         isAbsent = (sousBaremeDirectDoc.data() as Map<String, dynamic>?)?['isAbsent'] == true;
//         if (!isAbsent) {
//           storedValue = (sousBaremeDirectDoc.data() as Map<String, dynamic>?)?['Marks']?.toString() ?? '( - - - )';
//           print('✅ Valeur trouvée dans sous-barème direct ($sousBaremeId): $storedValue');
//         }
//       } else {
//         // 2. Chercher dans la collection imbriquée
//         var sousBaremeNestedDoc = await StudentsFirestoreHelper
//             .getSousBaremeMarksWithCache(
//           widget.currentUser.uid,
//           widget.classDocId,
//           studentId,
...
//         if (sousBaremeNestedDoc.exists) {
//           isAbsent = (sousBaremeNestedDoc.data() as Map<String, dynamic>?)?['isAbsent'] == true;
//           if (!isAbsent) {
//             storedValue = (sousBaremeNestedDoc.data() as Map<String, dynamic>?)?['Marks']?.toString() ?? '( - - - )';
//             print('✅ Valeur trouvée dans sous-barème imbriqué ($parentBaremeId/$sousBaremeId): $storedValue');
//           }
//         } else {
//           print('⚠️ Aucune valeur trouvée pour sous-barème $sousBaremeId');
//         }
//       }
//     } else {
//       // POUR LES BARÈMES PRINCIPAUX
...
//       if (baremeDoc.exists) {
//         isAbsent = (baremeDoc.data() as Map<String, dynamic>?)?['isAbsent'] == true;
//         if (!isAbsent) {
//           storedValue = (baremeDoc.data() as Map<String, dynamic>?)?['Marks']?.toString() ?? '( - - - )';
//           print('✅ Valeur trouvée pour barème $baremeKey: $storedValue');
//         }
//       } else {
//         print('⚠️ Aucune valeur trouvée pour barème $baremeKey');
//       }
//     }
...
//     final displayValue = _getDisplayEvaluationForTable(
//       storedValue,
//       evaluationSystem,
//       customNotes: customNotes,
//     );
...
//     print('   Valeur stockée: $storedValue');
//     print('   Notes personnalisées: $customNotes');
//     print('   Valeur affichée: $displayValue');
//     print('=== FIN DEBUG ===');
...
// // S'ASSURER QUE CETTE FONCTION EXISTE ET EST CORRECTE
...
//       case 'custom':
//         // IMPORTANT: Retourner les VRAIES NOTES PERSONNALISÉES
//         // Exemple: ["0", "1", "2", "3"] ou ["ضعيف", "مقبول", "جيد", "ممتاز"]
//         return customNotes.isNotEmpty
//             ? customNotes
//             : ['0', '1', '2', '3']; // Valeurs par défaut numériques
...
// // Mettre à jour la méthode _getSelectedValue dans le code 3
// // REMPLACER LA FONCTION EXISTANTE PAR CE CODE

// // AJOUTER CETTE FONCTION D'AIDE
//   String _getDefaultValueForSystem(
//       String evaluationSystem, List<String> customNotes) {
//     switch (evaluationSystem) {
//       case 'character':
//         return '( - - - )';
...
//       case 'custom':
//         return customNotes.isNotEmpty ? customNotes[0] : '( - - - )';

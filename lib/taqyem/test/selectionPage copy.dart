    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
            return Center(child: Text('Aucun élément sélectionné'));
          }
              ...
              return Column(
                children: [
                  ...
                  StreamBuilder<QuerySnapshot>(
                    stream: doc.reference.collection('sousBaremes').snapshots(),
                    builder: (context, sousSnapshot) {
                      if (sousSnapshot.connectionState == ConnectionState.waiting) {
                        return CircularProgressIndicator();
                      }
                      if (sousSnapshot.hasError) {
                        return Text('Erreur: ${sousSnapshot.error}');
                      }
                      ...
                      return Column(
                        children: sousBaremes.map((sousDoc) {
                          ...
                          return ListTile(
                            title: Text('Sous-Barème: $sousBaremeName'),
                            subtitle: Text(
                              isBaremeSelected 
                                ? 'Barème parent: $baremeName'
                                : 'Barème parent non sélectionné: $baremeName',
                              style: TextStyle(color: Colors.grey),
                            ),

  ...
  try {
    ...
    if (!baremeDoc.exists) {
      await baremeRef.set({
        ...
        'haveSoubarem': sousBaremeId != null, // Indique si des sous-barèmes existent
      });
    ...
    if (sousBaremeId != null) {
      ...
      if (newValue != null) {
        await sousBaremeDirectRef.set({'Marks': newValue}, SetOptions(merge: true));
      } else {
      ...
      var sousBaremeNestedRef = baremeRef.collection('sous_baremes').doc(sousBaremeId);
      if (newValue != null) {
        await sousBaremeNestedRef.set({'Marks': newValue}, SetOptions(merge: true));
      } else {
...
Future<void> _showSelectionsDialog(
  ...
  try {
    // Charger les sélections depuis le chemin users/{userId}/selections/{classId}/{matiereId}
    ...
    for (var doc in selectionsSnapshot.docs) {
      ...
      for (var sousDoc in sousBaremesSnapshot.docs) {
        String? savedEvaluation = await _getEvaluation(
          classId: classId,
          studentId: studentId,
          baremeId: doc.id, // ID du barème parent
          sousBaremeId: sousDoc.id, // ID du sous-barème
        );
      ...
      // Si aucun sous-barème n'existe, charger l'évaluation du barème principal
                        ...
                        onPressed: () async {
                          bool confirm = await showDialog(
                            ...
                            builder: (context) => AlertDialog(
                              ...
                              content: Text(
                                  'Êtes-vous sûr de vouloir enregistrer ces évaluations ?'),
                              actions: [
                                TextButton(
                                  onPressed: () =>
